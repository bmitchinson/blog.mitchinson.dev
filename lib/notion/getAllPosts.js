...
export async function getAllPosts({ includePages = false }) {
  const cacheKey = includePages ? "include-pages" : "posts-only";
  if (postsCache.has(cacheKey)) {
    return postsCache.get(cacheKey);
  }

  const postsPromise = getAllPostsUncached({ includePages });
  postsCache.set(cacheKey, postsPromise);
  return postsPromise;
}

async function getAllPostsUncached({ includePages = false }) {
  let id = BLOG.notionPageId;
  const authToken = BLOG.notionAccessToken || null;
  const api = new NotionAPI({ authToken });
  const response = await withNotionRetry(
    () => api.getPage(id),
    "loading the blog database"
  );
    ...
    if (!Object.keys(collectionQuery).length) {
      ...
      for (const viewId of rawMetadata.view_ids || []) {
        ...
        const collectionData = await withNotionRetry(
          ...
          `loading collection view ${viewId}`
        );

        ["block", "collection", "collection_view", "notion_user"].forEach(
          (table) => {
            response[table] = response[table] || {};
            Object.assign(response[table], collectionData.recordMap?.[table] || {});
          }
        );
        normalizeRecordMap(response);
